...
function applyAutoCorrect(inputElement) {
    ...
    const correctedLines = lines.map(line => {
        const words = line.split(/\s/); // / \s / is a regex - regular expression.
        const correctedWords = words.map(word => {
            const correctedWord = autoCorrectDictionary[word.toLowerCase()];
            return correctedWord ? correctedWord : word;
        }); //creates a new array where each is a corrected word
...
function saveCodeToLocalStorage() { // it is a built in function of the web storage api
...
//json = javascript object notation and is just like dictionaries in python consisting of key value pairs

function handleEnterKey(event) {
    if (event.key === "Enter") {
        event.preventDefault();
        const textarea = event.target;  //this triggers the element
